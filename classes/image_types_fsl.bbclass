...
SDCARD-A72 = "${IMGDEPLOYDIR}/${IMAGE_NAME}-A72.rootfs.sdcard"
...
generate_imx_sdcard () {
    ...
    parted -s ${SDCARD-A72} mklabel msdos
    parted -s ${SDCARD-A72} unit KiB mkpart primary fat32 ${IMAGE_ROOTFS_ALIGNMENT} $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED})
    parted -s ${SDCARD-A72} unit KiB mkpart primary $(expr  ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED}) $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED} \+ $ROOTFS_SIZE)
    parted ${SDCARD-A72} print
	...
	case "${IMAGE_BOOTLOADER}" in
		...
		;;
                imx-boot)
                dd if=${DEPLOY_DIR_IMAGE}/imx-boot-${MACHINE}.bin of=${SDCARD} conv=notrunc seek=${IMX_BOOT_SEEK} bs=1K
                ;;
                u-boot)
    ...
    dd if=${WORKDIR}/boot.img of=${SDCARD-A72} conv=notrunc,fsync seek=1 bs=$(expr ${IMAGE_ROOTFS_ALIGNMENT} \* 1024)
    dd if=${SDCARD_ROOTFS} of=${SDCARD-A72} conv=notrunc,fsync seek=1 bs=$(expr ${BOOT_SPACE_ALIGNED} \* 1024 + ${IMAGE_ROOTFS_ALIGNMENT} \* 1024)
}
...
IMAGE_CMD_sdcard () {
    ...
    dd if=/dev/zero of=${SDCARD-A72} bs=1 count=0 seek=$(expr 1024 \* ${SDCARD_SIZE})
